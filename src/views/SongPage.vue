<template>
  ...
  <el-dialog title="添加歌曲" v-model="centerDialogVisible">
    <el-form ref="songFormRef" id="add-song" label-width="120px" :model="registerForm">
...
async function getData() {
  ...
  if (route.query.type == "0") {
    result = (await HttpManager.getSongOfSingerId(singerId.value)) as ResponseBody;
  }else if (route.query.type == "1") {
    result = (await HttpManager.getSongOfSongListId(singerId.value)) as ResponseBody;
  }
...
function handleSongSuccess(res: any) {
  ElMessage({
    message: res.message,
...
// 更新图片
function handleImgSuccess(res: any, file: any) {
  ElMessage({
    message: res.message,
    type: res.type as any,
  });
...
const songFormRef = ref<InstanceType<typeof ElForm>>();

async function addSong() {
  // 使用Vue的ref获取表单元素
  const addSongForm = new FormData(songFormRef.value?.$el as HTMLFormElement);
  // 向表单数据中添加歌手 ID
  ...
  // 发送表单数据（包含文件上传）
  const res = await HttpManager.addSong(addSongForm) as ResponseBody;
